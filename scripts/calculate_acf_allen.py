...
def calculate_acf_sttc_t(sua_non_binned_l, n_lags_, resolution_ms_, fs_, duration_ms_=None):
    ...
    for row_idx, row in enumerate(sua_non_binned_l):
        ...
        spike_train = np.asarray(row[4:]).astype(float)
...
def fit_exp_t(acf_dict_in_out_, n_lags_, start_idx_=1):
    ...
    for k, v in acf_dict_in_out_.items():
        print('Processing animal_id: {}, unit_id: {}'.format(v['animal_id'], v['unit_id']))
        with warnings.catch_warnings():
            ...
            try:
                ...
                print('animal_id: {}, unit_id: {}: RuntimeError: {}'. format(v['animal_id'], v['unit_id'], e))
                v['popt'] = np.nan
                v['pcov'] = np.nan
                v['r_squared'] = np.nan
            except OptimizeWarning as o:
                print('animal_id: {}, unit_id: {}: OptimizeWarning: {}'. format(v['animal_id'], v['unit_id'], o))
                v['popt'] = np.nan
                v['pcov'] = np.nan
                v['r_squared'] = np.nan
            except RuntimeWarning as re:
                print('animal_id: {}, unit_id: {}: RuntimeWarning: {}'. format(v['animal_id'], v['unit_id'], re))
                v['popt'] = np.nan
                v['pcov'] = np.nan
                v['r_squared'] = np.nan
            except ValueError as ve:
                print('animal_id: {}, unit_id: {}: ValueError: {}'. format(v['animal_id'], v['unit_id'], ve))
