...
df <- read.csv("E:\\work\\q_backup_06_03_2025\\projects\\isttc\\results\\synthetic\\results\\param_fr_alpha_tau_comp_time\\full_signal\\var_len\\compute_time_full_long_df_1000_units.csv", 
...
df <- df %>%
  mutate(
    ...
    duration_s_s    = as.numeric(scale(duration_s))
  )
...
p10 <- p + scale_y_continuous(limits = c(-0.05, 0.2))
p10
...
new_duration <- new_duration %>%
  mutate(
    ...
    duration_s = duration_s_s * sd(df$duration_s,   na.rm=TRUE) + mean(df$duration_s,   na.rm=TRUE)
  )
