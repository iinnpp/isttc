...
n_lags = 20 # number of lags to calculate autocorrelation # 20
bin_size = int(50 * fs / 1000) # in ms # 50
sttc_dt_full = int(25 * fs / 1000) # dt for iSTTC on full signal and on concat trials, in ms # 25
...
results_folder = r"D:\param_fr_alpha_tau_comp_time\\trials\\"

# project_folder_path + 'results\\synthetic\\results\\param_fr_alpha_tau_comp_time\\full_signal\\'
...
if __name__ == "__main__":
    ...
    if calculate_acf_full:
        ...
        with open(dataset_folder + 'length450.pkl', 'rb') as f:
            data = pickle.load(f)
        ...
        acf_full_l = []
        acf_elapsed_time_l = []
        for idx, binned in enumerate(all_binned[:1000]):
            if idx % 100 == 0:
                print(f'[ACF] Processing unit {idx}')
            start = time.perf_counter()
            acf_unit = acf(binned, nlags=n_lags)
            elapsed_acf = time.perf_counter() - start
            acf_elapsed_time_l.append(elapsed_acf)
            acf_full_l.append(acf_unit)
        # get tau
        acf_results = {}
        for idx, acf_vals in enumerate(acf_full_l):
            ...
            acf_results[idx] = {
                ...
                'acf': acf_vals,
                'elapsed_time_acf': acf_elapsed_time_l[idx],
                'elapsed_time_tau_fit': elapsed_fit
            }
            if idx % 100 == 0:
                print(f'elapsed_acf {acf_elapsed_time_l[idx]}, elapsed_time_tau_fit {elapsed_fit}')
        # save
        with open(results_folder + 'acf_full_50ms_20lags_dict_450ms.pkl', "wb") as f:
    ...
    if calculate_sttc_full:
        ...
        isttc_full_l = []
        acf_elapsed_time_l = []
        for idx, st in enumerate(spike_trains[:1000]):
            if idx % 100 == 0:
                print(f'[iSTTC] Processing unit {idx}')
            st_int = np.asarray([int(spike) for spike in st])
            start = time.perf_counter()
            acf_unit = acf_sttc(st_int, n_lags, bin_size, sttc_dt_full, duration_ms[idx], verbose_=False)
            elapsed_acf = time.perf_counter() - start
            acf_elapsed_time_l.append(elapsed_acf)
            isttc_full_l.append(acf_unit)
            if idx % 10 == 0:
                print(f'elapsed_acf {elapsed_acf}')
        # get tau
        isttc_results = {}
        for idx, acf_vals in enumerate(isttc_full_l):
            ...
            if idx % 100 == 0:
                print(f'elapsed_acf {acf_elapsed_time_l[idx]}, elapsed_time_tau_fit {elapsed_fit}')
        # save
        with open(results_folder + 'acf_isttc_full_50ms_20lags_dict_450ms.pkl', "wb") as f:
            pickle.dump(isttc_results, f)

    # === Pearsonr Trial Avg ===
    if calculate_trials_pearsonr:
        print('[TRIAL] Starting Pearsonr trial avg...')
        with open(dataset_folder + 'trials100_binned.pkl', 'rb') as f:
        ...
        pearsonr_results = {}
        start_idx, stop_idx = 0, 1000 # len(trial_binned_dict)
        for i, (k, v) in enumerate(islice(trial_binned_dict.items(), start_idx, stop_idx), start=1):
            ...
            pearsonr_results[k] = {'taus': pearsonr_taus_l,
                                   ...
                                   'elapsed_time_acf': elapsed_acf_l,
                                   'elapsed_time_tau_fit': elapsed_fit_l}

        with open(results_folder + 'pearsonr_trial_avg_50ms_dict_100.pkl', "wb") as f:
            pickle.dump(pearsonr_results, f)

    # === iSTTC Trial Avg ===
    if calculate_trials_sttc_avg:
        print('[TRIAL] Starting iSTTC trial avg...')
        with open(dataset_folder + 'trials60.pkl', 'rb') as f:
        ...
        isttc_avg_results = {}
        start_idx, stop_idx = 0, 1000 # len(trial_dict)
    ...
    if calculate_trials_sttc_concat:
        ...
        with open(dataset_folder + 'trials100.pkl', 'rb') as f:
            trial_dict_full = pickle.load(f)
        trial_dict = trial_dict_full['trial_dict']
        n_trials_all = trial_dict_full['n_trials']
        trial_lens_all = trial_dict_full['trial_lens']
        ...
        with open(results_folder + 'sttc_trial_concat_50ms_dict_100.pkl', "wb") as f:
